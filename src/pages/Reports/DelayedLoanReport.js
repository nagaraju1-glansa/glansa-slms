...
const columns = [
  ...
  { name: 'M No.', selector: row => row.mno, sortable: true },
  { name: 'M Name', selector: row => row.member_name, sortable: true },
  { name: 'Purpose', selector: row => row.purpose || 'Loan', sortable: true },
...
const DelayedLoanReport = ({ data }) => {
  const exportToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Delayed Loan Report');
    const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const file = new Blob([excelBuffer], { type: 'application/octet-stream' });
    saveAs(file, 'DelayedLoanReport.xlsx');
  };
  ...
  const exportToPDF = () => {
    ...
    doc.save('DelayedLoanReport.pdf');
  };
...
export default DelayedLoanReport;
