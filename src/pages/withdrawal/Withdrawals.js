...
const Withdrawals = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const res = await CustomFetch(`/member-details`);
  ...
  const columns = [
    ...
    { name: 'Savings', selector: row => row.total_saving, sortable: true },
    {
      ...
      cell: row => (
       <div>
         <Link to ={`/withdrawaledit/${row.m_no_encpt}`}
     ...
     <React.Fragment>
        ...
        <div>
          <div className='page-title-box d-sm-flex align-items-center justify-content-between'>
              ...
              <button type="button" className="btn btn-success waves-effect waves-light"  onClick={()=> navigate('/withdrawaladd') }>
